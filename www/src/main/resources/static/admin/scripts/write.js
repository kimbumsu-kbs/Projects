...
// region XHR
ClassicEditor.create($main['description'], editorConfig).then((editor) => {
    $main.onsubmit = (e) => {
        e.preventDefault();
        if ($main['title'].value === '') {
        ...
        if (!$main['startDate'].value || $main['startDate'].value === null) {
            alert('축제 시작일을 입력하지 않았습니다. 시작일을 입력해주세요.');
            return;
        }
        if (!$main['endDate'].value || $main['endDate'].value === null) {
            alert('축제 종료일을 입력하지 않았습니다. 종료일을 입력해주세요.');
            return;
        }
        if (new Date($main['startDate'].value) > new Date($main['endDate'].value)) {
            alert('시작일은 종료일보다 늦을 수 없습니다.');
            return;
        }
        if (!$main['coverData'].files || !$main['coverData'].files.length) {
            alert('배너 이미지를 추가하지 않았습니다. 이미지를 추가 해주세요.');
            return;
        }
        const allowedMimeTypes = ['image/jpeg', 'image/png', 'image/gif'];
        const file = $main['coverData'].files[0];
        ...
        formData.append('startDate', $main['startDate'].value);
        formData.append('endDate', $main['endDate'].value);
        formData.append('coverData', $main['coverData'].files[0]); // 파일 객체
